...
export function AdminDashboard() {
  ...
  const handleCheckIn = (patientId:any) => {
        ...
        <Card className="w-full bg-white shadow-md" key={index}>
          <CardContent className="grid grid-cols-[1fr_1fr_1fr] gap-4 p-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="text-gray-800">{patient?.patient?.reason || 'No reason provided'}</div>
            </div>
            <div className="space-y-1.5">
  ...
  const renderPatientCheckIn = () => (
    <div className="space-y-6">
      ...
      {filteredPatients.map((patient) => (
        <Card key={patient.id} className="w-full bg-white shadow-sm">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="grid grid-cols-2 gap-4 flex-grow">
                ...
                <div>
                  ...
                  <div className="text-lg font-semibold text-gray-900">{patient.abhaId}</div>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-500">Symptom</Label>
                  <div className="text-lg font-semibold text-gray-900">{patient.symptom}</div>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-500">Time Slot</Label>
                  <div className="text-lg font-semibold text-gray-900">{patient.timeSlot}</div>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-500">Date</Label>
                  <div className="text-lg font-semibold text-gray-900">{patient.date}</div>
                </div>
              </div>
              <div className="flex gap-2">
                <Button onClick={() => handleCheckIn(patient.id)} className="bg-green-500 hover:bg-green-600 text-white">
                  Check In
                </Button>
                <Button onClick={() => handleCancel(patient.id)} variant="outline" className="border-red-500 text-red-500 hover:bg-red-50">
                  Cancel
                </Button>
              </div>
            </div>
        ...
        <div className="w-64 bg-white shadow-md">
          <nav className="mt-5 px-2">
            ...
            <Button
              variant="ghost"
              className="w-full justify-start mb-1 text-left font-normal"
              onClick={() => setActiveView('bedAllocation')}
            >
            ...
            <Button
              variant="ghost"
              className="w-full justify-start mb-1 text-left font-normal"
              onClick={() => setActiveView('patientCheckin')}
            >
